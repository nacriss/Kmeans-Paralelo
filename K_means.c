/* CPU PARALELO TEMPOS POR THREADS  
1             28,61
2             15,17
4             8,41
8             8,53
 ...
 * centroid
 */
//não compensa paralelizar pq é "simples", não tem um custo computacional pesado...
...
cluster *kMeans(observation observations[], size_t size, int k)
{
    ...
    else if (k < size)
    {
        ...
        do
        {
            ...
            /* STEP 2*/
            // recalcula os centroides, sem paralelizar
            for (size_t j = 0; j < size; j++)
            {
            ...
            #pragma omp parallel for reduction(+:changed) private(t) // race condition em changed e varíável t foi criada antes do bloco, portanto deve ser privatizada
            for (size_t j = 0; j < size; j++)
            {
...
observation *loadDataset(const char *filename, size_t *size)
{
    ...
    observation *observations =
        malloc(capacity * sizeof(observation));
    ...
    *size = 0;
    ...
    while (fgets(line, sizeof(line), file))
    {
        ...
        if (*size >= capacity)
        {
            ...
            observations =
                realloc(observations,
                        capacity * sizeof(observation));
        }
...
void testAccidentsDataset()
{
    ...
    observation *observations =
        loadDataset("US_Accidents_March23.csv", &size);
    ...
    // vai agrupar acidentes por estados, então um numero de clusters pequeno não faria sentido
    int k = 50;

    double start = omp_get_wtime();

    cluster *clusters =
        kMeans(observations, size, k);
